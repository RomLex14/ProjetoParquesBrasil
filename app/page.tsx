...
import { useEffect, useState, useRef } from "react" // <<< 1. ADICIONADO 'useRef'
...
export default function HomePage() {
  ...
  // <<< 3. NOVOS ESTADOS E REF PARA A BUSCA DINÂMICA >>>
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchQuery.trim()) {
      setShowSuggestions(false) // Esconde sugestões ao submeter
      router.push(`/search?q=${encodeURIComponent(searchQuery.trim())}`)
    }
  }
  
  // <<< 4. LÓGICA PARA BUSCAR SUGESTÕES EM TEMPO REAL >>>
  useEffect(() => {
      ...
      const fetchSuggestions = async () => {
        ...
        try {
          const [trilhasRes, parquesRes] = await Promise.all([
            supabase.from('trilhas').select('id, nome, parques(localizacao)').ilike('nome', `%${searchQuery}%`).limit(3),
          ...
          const trilhasSuggestions: Suggestion[] = (trilhasRes.data || []).map((t: any) => ({
            ...
            nome: t.nome,
            type: 'trilha',
            location: t.parques?.localizacao || 'Trilha',
            link: `/trilhas/${t.id}`
          }));

          const parquesSuggestions: Suggestion[] = (parquesRes.data || []).map(p => ({
            id: p.id,
            nome: p.nome,
            type: 'parque',
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-4">
          ...
          <div ref={searchContainerRef} className="relative w-full max-w-2xl">
            ...
            {showSuggestions && searchQuery.length > 1 && (
              <div className="absolute top-full mt-2 w-full bg-background rounded-xl shadow-lg border text-left overflow-hidden">
        ...
        <div className="text-center mb-12">
          ...
          <div className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            {!isClient || loadingAuth ? (
              ...
              "Baseado nas suas preferências, selecionamos essas trilhas especiais para sua próxima aventura."
            ) : (
      ...
      {/* A seção Premium e o Footer foram omitidos por brevidade, mas continuam no seu código */}
